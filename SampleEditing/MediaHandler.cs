...
public class MediaHandler
{
    ...
    public MediaHandler()
    {
        System.Diagnostics.ProcessStartInfo pg = new ProcessStartInfo();
    ...
    public VideoInfo Split_Video(int length, int total_clips, string startSecond)
    {
        ss = startSecond;
        VideoInfo info = new VideoInfo();
        string output_name = "";
        ...
        try
        {
            int i = 0;
            string _index = "";
            int start_index = (int)Double.Parse(ss);//0
            Start_Position = ss;
            Duration = length.ToString();
            string output = "";
            if (no_of_clips == 1)
    ...
    private string Process_CMD(string _ffmpegpath, string cmd)
    {
        ...
        Process _process = new Process();
        //_process.StartInfo.Arguments = "-ss "+ss+" -t "+_duration+" ";
        _process.StartInfo.UseShellExecute = false;
        _process.StartInfo.RedirectStandardInput = true;
        //_process.StartInfo.RedirectStandardOutput = true;
        _process.StartInfo.RedirectStandardError = true;
        ...
        //var query = _process.StartInfo.Arguments.Substring(0, 4) + " " + ss + " " + _process.StartInfo.Arguments.Substring(7);
        //_process.StartInfo.Arguments = query;
        if (_process.Start())
        {
    ...
    private string Prepare_Command(string inputpath, string outputpath, bool isaudio, bool iswatermark)
    {
        ...
        //Start_Position (int)Double.Parse(ss)
        // set audio codecs
        string _frc = "";
        if (Force != "")
            _frc = " -f " + Force + " ";
